                                         ...
                                         <div class=" col-md-4 text-center"></div>
                                            <div class="col-md-4 text-center">
                                                <div class="detail-gallery">
                                                    <img :src="(urlimage+imagePath)" alt="kidify">
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                       <h4> <label style="font-size: 18px"  for="exampleInputEmail1">Libelle*</label></h4>
                                                        <input style="font-size: 20px" v-model="libelle" type="text" class="form-control form-account" name="" placeholder="Nom du produit*">
                                                    </div>
                                                </div>

                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <h4> <label style="font-size: 18px" for="exampleInputEmail1">Catégories*</label></h4>
                                                        <v-select  style="background-color:beige width:200px; font-size: 20px" class="default-form form-control-xs"  v-model="selected1" :reduce="(option) => option.id" :options=options1 />
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <h4> <label style="font-size: 18px"  for="exampleInputEmail1">Contenu *</label></h4>
                                                ...
                                                <button @click="submit" type="submit" class="btn btn-prim uppercase" > Mettre à jour .. </button>
...
import axiosClient from "../../../ApiServices";
    ...
    const { getlisteCategories,getCategoriesById,CategoriesDetail,NblisteCategories,listeCategories,Messages,getArticlesById,ArticlesById} = useBlogs();
    onMounted(async () => {
    ...
    const libelle=ref('');
    const posts_id=ref('');
    const posts_slug=ref('');
    const posts_user_id=ref('');
    const imagePath=ref('');
    ...
    const createImage = async (item) => {
        ...
        postFormData.append('posts_id', posts_id.value);
        postFormData.append('libelle', libelle.value);
        postFormData.append('images', attachments.value);
        postFormData.append('selected1', selected1.value);
        postFormData.append('posts_title', libelle.value);
        postFormData.append('description', description.value);
        postFormData.append('posts_user_id', posts_user_id.value);
        showBtn.value = 1;
    };
    ...
    const submit = async (valeur) => {
             ...
             if (libelle.value.length > 0 ) {
                ...
                if(attachments.value){
                    console.log('Bonjour');
                         axiosClient.post('/api/updateArticles', postFormData, config, {})
                          .then(response => {
                            ...
                            setInterval(() => {
                                success.value = false;
                                message.value = "";
                                // getArticlesById(route.params.id);
                                router.go();
                    ...
                    console.log('Bonsoir')
                    axiosClient.post('/api/updateArticles', {
                        posts_id:posts_id.value,
                        libelle:libelle.value,
                        posts_slug:posts_slug.value,
                        posts_user_id:posts_user_id.value,
                        description:description.value,
                        category_id:selected1.value,
    ...
    watchEffect(() => {
        ...
        if(ArticlesById.value.length) {
            let tables1 = [];

            ArticlesById.value.map(function (element) {
                posts_id.value=element.posts_id;
                libelle.value=element.posts_title;
                posts_slug.value=element.posts_slug;
                posts_user_id.value=element.posts_user_id;
                description.value=element.posts_body;
                category_id.value=element.posts_category_id;
                selected1.value=element.posts_category_id;
